...
#### Data train/test data splits based on multi-class output *type*:
...
#brain_train_idx <- createDataPartition(brain$type, p=0.67, list = FALSE)
...
#brain_pcaLRtest <- preProcess(brain_testLR[,-c(1:3)],
#                    method = c("pca"))
#brain_pcaLRtest
...
#### Multinomiall target variable datasets

#### BoxCox, Scale and Center Training data **Mulitinomial** target variable "type":
...
#### PCA on training data **Mulitnomial** target variable:

```{r pcatrainmultinomial data}

#brain_pcatrain <- preProcess(brain2_train[,-c(1:2)],
#                    method = c("pca"))
#brain_pcatrain

#brain_train_pca <- predict(brain_pcatest, brain2_train)
```


#### Multinomial Test Dataset processing 
(to avoid any leakage of training data onto the test data)

#### BoxCox, center and scale test data **Multinomial** target variable:
...
#### PCA on test data **Mulitnomial** target variable:

```{r pcatestmultinomial data}

#brain_pcatest <- preProcess(brain2_test[,-c(1:2)],
#                    method = c("pca"))
#brain_pcatest

#brain_test_pca <- predict(brain_pcatest, brain2_test)
```
...
ctrl_brain2 <- trainControl(method = "LOOCV",
                     classProbs = FALSE,
                     savePredictions = TRUE)
...
#nsc_model_brain2 <- train(x = brain_train1[,-c(1:2)], 
#                y = brain_train1$type,
#                method = "pam",
...
# This accuracy barely surpsasses this baseline accuracy requirement.
```
```{r checks_rf}
...
#nnRoc <- roc(response = NN_pred$pred$obs,
#             predictor = NN_pred$pred$No,
#             levels = rev(levels(NN_pred$pred$obs)))

#ldaRoc <- roc(response = rf_predict$pred$obs,
#             predictor = rf_predict$pred$No,
#             levels = rev(levels(rf_predict$pred$obs)))

#plsdaRoc <- roc(response = PLSDA_pred$pred$obs,
#             predictor = PLSDA_pred$pred$No,
#             levels = rev(levels(PLSDA_pred$pred$obs)))

#nscRoc <- roc(response = nsc_pred$pred$obs,
#             predictor = nsc_pred$pred$No,
#             levels = rev(levels(nsc_pred$pred$obs)))

#lrRoc <- roc(response = lr_pred$pred$obs,
#             predictor = lr_pred$pred$No,
#             levels = rev(levels(lr_pred$pred$obs)))

#svmRoc <- roc(response = svm_predict$pred$obs,
#             predictor = svm_predict$pred$No,
#             levels = rev(levels(svm_predict$pred$obs)))

#plot(svmRoc, type = "s", col = 'pink', legacy.axes = TRUE)
#plot(rfRoc, type = "s", col = 'orange', legacy.axes = TRUE)
...
#legend("bottomright", legend=c("SVM", "RF", "LR", "PLSDA", "NN", "NSC"),
#       col=c("pink", "orange", "red", "green","blue", "black"), lwd=2)
